...
fn get_web3_transport(chain: &str) -> Result<Web3<web3::transports::Http>, Box<dyn Error>> {
    let rpc_endpoint = match chain {
        ...
        _ => Err(IoError::new(ErrorKind::Other, format!("invalid chain id"))),
    }?;
    let transport = web3::transports::Http::new(rpc_endpoint.as_str())?;
    Ok(web3::Web3::new(transport))
}

async fn web3_get_balance(chain: &str, address: &str) -> Result<Decimal, Box<dyn Error>> {
    let web3 = get_web3_transport(chain)?;
